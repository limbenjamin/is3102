...
@ManagedBean
@ViewScoped
public class IngredientPlannedPOManagedBean implements Serializable {
    ...
    public void init() {
        ...
        if (purchaseOrderId != null || !(staff.getPlant() instanceof Store)) {
            ...
            ingredSupplier = purchaseOrder.getIngredSupplier();
    ...
    public void addStock() throws ParseException {
        ...
        ingredientId = Long.parseLong(request.getParameter("createPODetail:ingredientId"));
        numberOfLots = Integer.parseInt(request.getParameter("createPODetail:numberOfLots"));
        try {
            ipml.createNewIngredPurchaseOrderDetail(purchaseOrderId, ingredientId, numberOfLots);
            purchaseOrderDetailList = ipml.viewIngredPurchaseOrderDetails(purchaseOrderId);
            FacesContext.getCurrentInstance().getExternalContext().getFlash().put("message",
                    new FacesMessage(FacesMessage.SEVERITY_INFO, "Ingredient added successfully!", ""));
    ...
    public void sendEmail(IngredientSupplier recipient) {
        String title = "[Confirmed Ingredient Purchase Order: #" + purchaseOrderId + "] from Island Furniture "
                + store.getName();
        String orderContent = store.getName() + " Restaurant would like to order the following: " + "\r\n ";
        ...
        try {
            ...
            Logger.getLogger(IngredientPlannedPOManagedBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    }    
    ...
    public String editStock(ActionEvent event) throws IOException {
        IngredientPurchaseOrderDetail pod = (IngredientPurchaseOrderDetail) event.getComponent().getAttributes().get("PODid");
        System.out.println("Ingredient Purchase Order Detail Id is: " + pod.getId().toString());
        
        ipml.updateIngredPurchaseOrderDetail(pod);
        purchaseOrder = ipml.getPurchaseOrder(purchaseOrderId);
        FacesContext.getCurrentInstance().getExternalContext().getFlash().put("message",
                new FacesMessage(FacesMessage.SEVERITY_INFO, "Update Successful!", ""));

        return "ingredientplannedpo";
    }    
    ...
    public String deletePurchaseOrderDetail() {
        ...
        ipml.deleteIngredPurchaseOrderDetail(purchaseOrderDetailId);
        purchaseOrderDetailList = ipml.viewIngredPurchaseOrderDetails(purchaseOrderId);
        FacesContext.getCurrentInstance().getExternalContext().getFlash().put("message",
